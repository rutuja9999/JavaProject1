...
public class EmployeeRepository implements EmployeeCrudRepository {
	...
	public boolean add(Employee employee){
		String query="INSERT INTO employee_database values(?,?,?,?,?,?,?)";
		int rowAdded=0;
	...
	public List<String> getNameAndPhoneNumber() throws EmployeeNotFoundException {
        ...
        String query = "SELECT first_name,phone_number FROM employee_database";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
	...
	public boolean updateEmailAndPhoneNumber(String initialMail, String changedMail, long phoneNumber) throws EmployeeNotFoundException {
		String query="UPDATE employee_database SET email_address=?,phone_number=? WHERE email_address=?";
		int rowUpdated=0;
		try(PreparedStatement statement=connection.prepareStatement(query)){
	...
	public boolean deleteByName(String name,String email) throws EmployeeNotFoundException {
		String query="DELETE FROM employee_database WHERE first_name=? AND email_address=?";
		int rowDeleted=0;
		try(PreparedStatement statement=connection.prepareStatement(query)){
	...
	public List<String> getNameAndEmail(LocalDate date) throws EmployeeNotFoundException {
		List<String> employeesList = new ArrayList<>();
        Employee employee=null;
        String query = "SELECT first_name,email_address FROM employee_database WHERE MONTH(date_of_birth)=? AND DAY(date_of_birth)=?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (resultSet.next()) {
                ...
                employeesList.add("Employee Name : "+employee.getFirstName()+" Employee Email : "+employee.getEmailId());
                while (resultSet.next()) {
                    employee = mapRowToObject(resultSet,"NameAndEmail");
                    employeesList.add("Employee Name : "+employee.getFirstName()+" Employee Email : "+employee.getEmailId());
                }
	...
	public List<String> getNameAndPhoneNumber(LocalDate date) throws EmployeeNotFoundException {
		List<String> employeesList = new ArrayList<>();
        Employee employee=null;
        String sql = "select first_name,phone_number from employee_database where MONTH(wedding_date)=? AND DAY(wedding_date)=?";
	...
	public List<Employee> findAll() throws EmployeeNotFoundException{
		...
		String query="SELECT * FROM employee_database";
		Employee employee=null;
		try(PreparedStatement statement=connection.prepareStatement(query)){
			ResultSet resultSet=statement.executeQuery();
			if(resultSet.next()) {
